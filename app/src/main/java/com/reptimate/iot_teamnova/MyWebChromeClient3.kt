...
class MyWebChromeClient3 (private val activity: BoardWebViewActivity) : WebChromeClient() {
...
//                            mFilePathCallback?.onReceiveValue(result?.let { arrayOf(it) })
    ...
    override fun onShowFileChooser(webView: WebView?, filePathCallback: ValueCallback<Array<Uri>?>?, fileChooserParams: FileChooserParams?): Boolean {
        ...
        when {
            ContextCompat.checkSelfPermission(
                ...
                val startCustomAlbum = Intent(activity, CustomAlbumActivity2::class.java)
                activityForResult.launch(startCustomAlbum)
            }

            activity.shouldShowRequestPermissionRationale(android.Manifest.permission.READ_EXTERNAL_STORAGE) -> {
        ...
        when (requestCode) {
            0 -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    //권한 허용클릭
                    //TODO()앨범으로 이동시키기!
                } else if (activity.shouldShowRequestPermissionRationale(android.Manifest.permission.READ_EXTERNAL_STORAGE)) {
                    //권한 처음으로 거절 했을 경우
                    //한번더 권한 요청
                    showPermissionAlertDialog()
                } else {
    ...
    private fun goSettingActivityAlertDialog() {
        AlertDialog.Builder(activity)
            ...
            .setPositiveButton("허용하러 가기") { _, _ ->
                ...
                goSettingPermission.data = Uri.parse("package:${activity}packageName")
                activity.startActivity(goSettingPermission)
            }
    ...
    private fun getResultUri(uri: Uri?): Uri? {
        return uri?.let {
            val filePath: String? =
                "file:" + RealPathUtil.getRealPath(activity, it)
